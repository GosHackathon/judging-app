...
const JudgeModel = require('../models/Judge'); // Import Judge model
const JudgeGroupModel = require('../models/JudgeGroup'); // Import JudgeGroup model
...
const insertTeams = async (teams) => {
  try {
    for (let team of teams) {
      console.log('Processing team:', team); // Debugging line
      ...
      if (!teamName || !teamEmail) {
        ...
        continue; // Skip this team if required fields are missing
      }
      ...
      if (!existingTeam) {
        const newTeam = new TeamModel({
          teamName: teamName,
          teamEmail: teamEmail, // Ensure field is correctly saved
...
const insertOrUpdateJudges = async (judges) => {
  try {
    for (const judge of judges) {
      console.log('Processing judge:', judge); // Debugging line

      const judgeName = judge['JudgeName']; // Update property name
      const judgeEmail = judge['JudgeEmail']; // Update property name
      ...
      await JudgeModel.findOneAndUpdate(
  ...
  try {
    ...
    for (const group of judgeGroups) {
      ...
      if (JudgeName && JudgeEmail) {
        ...
        continue; // Skip this entry if judge data is missing
      }

      // Add team to the group
      if (teamName) { // Only check for teamName
        let team = await TeamModel.findOne({ teamName: teamName.trim() });
        if (team) {
...
exports.uploadTeamFile = async (req, res) => {
  try {
    ...
    if (!teamSheet || !judgeSheet || !judgeGroupSheet) {
      console.error('Sheet names are incorrect or missing in the uploaded file');
